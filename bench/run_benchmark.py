...
Usage:
    python -m bench.run_benchmark --workdir <dir> [--corpus builtin|adversarial|all]
...
def build_corpus(workdir: Path, which: str) -> Path:
    cases = workdir / "cases"
...
def main() -> int:
    ...
    parser.add_argument("--corpus", default="all",
                        choices=("builtin", "adversarial", "all"))
    parser.add_argument("--predictor", default="mock", choices=("mock", "cli"))
    args = parser.parse_args()

    workdir = Path(args.workdir)
    corpus = build_corpus(workdir, args.corpus)

    predictor: Predictor
    if args.predictor == "cli":
        # S5 CLI contract pending; fails loudly rather than faking results.
        predictor = CliPredictor("semlock")
    else:
